...
class Register extends Component {
  ...
  handleSubmit = () => {
    ...
    form.validateFields((err, values) => {
      if (!err) {
        postRequest(baseUrl,values,res=>{
          if(res){
            const account = form.getFieldValue('userEmail');
            router.push({
              pathname: '/user/register-result',
              state: {
                account,
  ...
  checkNameSize = (rule,value,callback) => {
    const { form } = this.props;
    let username =  form.getFieldValue('userName');
    if( username.length > 10){
      callback(formatMessage({ id: 'validation.userName.size' }))
    } else {
      callback();
    }
  };
